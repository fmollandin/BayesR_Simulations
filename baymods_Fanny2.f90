...
!    along with this program.  If not, see <http://www.gnu.org/licenses/>
 ...
 module parz
  ...
  logical :: mcmc, snpout, permute, cat, beta
  !File names
  character(len=200) :: genfil, phenfil, bimfil, inprefix, outprefix, logfil, freqfil, &
                        mbvfil, hypfil, locfil, modfil, paramfil, betafil, catfil
  ! Data
...
subroutine parse_out
  ...
  do i=1,nopt
    if(str_match(trim(register%key(i)),'-out')) then
        ...
        catfil= trim(outprefix)//'.catit'
        betafil= trim(outprefix)//'.beta'
...
subroutine parse_initialise
  ...
  do i=1,nopt
     if(str_match(trim(register%key(i)),'-msize')) then
        ...
        beta= cast_logical(register%default(i))
     else if(str_match(trim(register%key(i)),'-cat')) then
        cat = cast_logical(register%default(i))
...
subroutine allocate_data
  ...
  allocate(pred(nind), gpin(ndist), gp(ndist), p(ndist), &
           ...
           snptracker(nloci), log_p(ndist), log_gp(ndist),vare_gp(ndist), varustore(nloci),  stat=ios)
  if( ios /= 0 )then
...
subroutine output_model
  ...
  write(10,'(2(A5))',advance="no") ' ','beta'
  write(10,*) '    variance'
  do i=1,nloci
     write(10,'(50(E15.7,1X))') indiststore(i,:), gstore(i), varustore(i)
  enddo
...
subroutine output_cat
  integer :: i
  character(LEN=10) :: cati
   do i=1,1000
     write(cati,'(I8)') snptracker(i)
     cati=adjustl(cati)
     write(100,'(1X,a)',advance='no') trim(cati)
  enddo
  write(100,*)
end subroutine output_cat

subroutine output_beta
  integer :: i
  character(LEN=100) :: betai
  do i=1,1000
     write(betai,'(e15.6)') g(i)**2
     betai=adjustl(betai)
     write(101,'(1X,a)',advance='no') trim(betai)
  enddo
  write(101,*)
